...
export const PostIdProvider = ({children})=>{
    ...
    useEffect(()=>{
        if(deletePost){ //as i mentioned in detailPage component, the deletion process will begin as the value set to true
            try{
                axios.delete(`http://localhost:3030/posts/${currentPostId}`)//api call to delete
    ...
    useEffect(()=>{
        ...
        const fetchUser = async ()=>{
            try{
                ...
                const data = await axios.get(`http://localhost:3030/posts/${currentPostId}?_expand=user`).then(res=>res.data)
