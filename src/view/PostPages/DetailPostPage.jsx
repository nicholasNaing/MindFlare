...
function DetailPostPage() {
  ...
  const fetchDeatilPost = async ()=>{
    ...
    const data = await axios.get(`http://localhost:3030/posts/${postId.id}`).then(res=>res.data)
    //to put the author name below post title, had to call another api to get the postAutor data
    const userName = await axios.get(`http://localhost:3030/users/${data.userId}`).then(res=>res.data)
