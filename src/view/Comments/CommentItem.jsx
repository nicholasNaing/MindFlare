...
function CommentItem({comment}) {
  ...
  useEffect(()=>{
    const fetchCommentor = async()=>{
      try{
        const commentOwner = await axios.get(`http://localhost:3030/comments/${comment.id}?_expand=user`).then(res=>res.data)
  ...
  function deleteComment(commentorId,commentId){
    ...
    if(userId === commentorId){
      axios.delete(`http://localhost:3030/comments/${commentId}`) //delete the commend 
