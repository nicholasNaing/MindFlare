...
export async function emailExist(email,updateEmail){  //function to check if email is in database
    ...
    try {
        const checkEmail = await axios.get(`http://127.0.0.1:3030/users?email=${email}`).then(res=>res.data)  //get the data in database with the given email
...
export async function phoneExist(phone,updatePhone){  //function to check if phone is in database
    ...
    try {
        ...
        const checkPhone = await axios.get(`http://127.0.0.1:3030/users?phone=%2b${encodedPhone}`).then(res=>res.data)  //get the data in database with the given phone
